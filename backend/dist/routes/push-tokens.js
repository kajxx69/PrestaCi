...
router.post('/', async (req, res) => {
    try {
        ...
        const [existingRows] = await pool.query('SELECT id FROM push_tokens WHERE user_id = ? AND token = ? LIMIT 1', [userId, token]);
        if (existingRows.length > 0) {
            // Mettre à jour le token existant
            await pool.query('UPDATE push_tokens SET device_type = ?, device_id = ?, is_active = 1, updated_at = NOW() WHERE id = ?', [device_type, device_id, existingRows[0].id]);
            res.json({ ok: true, message: 'Token mis à jour' });
        }
        else {
            // Créer un nouveau token
            const [result] = await pool.query('INSERT INTO push_tokens (user_id, token, device_type, device_id, is_active, created_at, updated_at) VALUES (?, ?, ?, ?, 1, NOW(), NOW())', [userId, token, device_type, device_id]);
...
router.get('/', async (req, res) => {
    try {
        const userId = req.userId;
        const [rows] = await pool.query('SELECT id, token, device_type, device_id, is_active, created_at, updated_at FROM push_tokens WHERE user_id = ? ORDER BY created_at DESC', [userId]);
        res.json(rows);
    }
    catch (e) {
...
router.put('/:id/toggle', async (req, res) => {
    try {
        ...
        const [tokenRows] = await pool.query('SELECT id, is_active FROM push_tokens WHERE id = ? AND user_id = ? LIMIT 1', [tokenId, userId]);
        if (tokenRows.length === 0) {
            return res.status(404).json({ error: 'Token introuvable' });
        }
        const newStatus = tokenRows[0].is_active ? 0 : 1;
        await pool.query('UPDATE push_tokens SET is_active = ?, updated_at = NOW() WHERE id = ?', [newStatus, tokenId]);
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        const tokenId = parseInt(req.params.id);
        // Vérifier que le token appartient à l'utilisateur
        const [result] = await pool.query('DELETE FROM push_tokens WHERE id = ? AND user_id = ?', [tokenId, userId]);
...
router.post('/cleanup', async (req, res) => {
    try {
        ...
        const [result] = await pool.query('DELETE FROM push_tokens WHERE user_id = ? AND is_active = 0 AND updated_at < DATE_SUB(NOW(), INTERVAL 30 DAY)', [userId]);
