...
router.get('/', requireAuth, async (req, res) => {
    try {
       ...
       ORDER BY r.date_reservation DESC, r.heure_debut DESC`, [userId]);
        res.json(rows);
    }
    catch (e) {
...
router.put('/:id/cancel', requireAuth, async (req, res) => {
    try {
        ...
        const [rows] = await pool.query(`SELECT r.*, s.nom AS statut_nom FROM reservations r JOIN statuts_reservation s ON r.statut_id = s.id WHERE r.id = ? AND r.client_id = ? LIMIT 1`, [id, userId]);
        const r = rows[0];
        if (!r)
        ...
        const [sRows] = await pool.query(`SELECT id FROM statuts_reservation WHERE nom = 'annulee' LIMIT 1`);
        const cancelledId = sRows[0]?.id;
        if (!cancelledId)
            return res.status(500).json({ error: 'Statut annulee introuvable' });
       ...
       VALUES (?, ?, ?, ?, ?)`, [id, r.statut_id, cancelledId, 'Annulation par le client', userId]);
        res.json({ ok: true });
    }
    catch (e) {
        res.status(500).json({ error: e.message });
...
router.post('/', requireAuth, async (req, res) => {
    try {
        ...
        const [sRows] = await pool.query('SELECT * FROM services WHERE id = ? AND is_active = 1', [service_id]);
        ...
        const [stRows] = await pool.query(`SELECT id FROM statuts_reservation WHERE nom = 'en_attente' LIMIT 1`);
        const enAttenteId = stRows[0]?.id;
        if (!enAttenteId)
            return res.status(500).json({ error: 'Statut par défaut introuvable' });
        // 4. Insert reservation
        const [iResult] = await pool.query(`INSERT INTO reservations (client_id, prestataire_id, service_id, statut_id, date_reservation, heure_debut, heure_fin, prix_final, notes_client, a_domicile, adresse_rdv, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`, [userId, service.prestataire_id, service_id, enAttenteId, date_reservation, heure_debut, heure_fin, service.prix, notes_client, a_domicile, a_domicile ? adresse_rdv : null]);
        const reservationId = iResult.insertId;
        // 5. Log history
        await pool.query(`INSERT INTO historique_reservations (reservation_id, ancien_statut_id, nouveau_statut_id, commentaire, changed_by_user_id)
       VALUES (?, ?, ?, ?, ?)`, [reservationId, null, enAttenteId, 'Création de la réservation', userId]);
