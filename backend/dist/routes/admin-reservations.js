...
router.put('/:id/status', async (req, res) => {
    try {
        ...
        const [reservations] = await pool.execute(`SELECT r.*, uc.nom as client_nom, p.nom_commercial as prestataire_nom, s.nom as service_nom
       FROM reservations r
       LEFT JOIN users uc ON r.client_id = uc.id
       LEFT JOIN prestataires p ON r.prestataire_id = p.id
       LEFT JOIN services s ON r.service_id = s.id
       WHERE r.id = ?`, [id]);
        ...
        await pool.execute('UPDATE reservations SET statut = ?, updated_at = NOW() WHERE id = ?', [statut, id]);
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        // Vérifier que la réservation existe
        const [reservations] = await pool.execute(`SELECT r.*, uc.nom as client_nom, up.nom as prestataire_nom, s.nom as service_nom
       FROM reservations r
       LEFT JOIN users uc ON r.client_id = uc.id
       LEFT JOIN users up ON r.prestataire_id = up.id
       LEFT JOIN services s ON r.service_id = s.id
       WHERE r.id = ?`, [id]);
        ...
        await pool.execute('DELETE FROM avis WHERE reservation_id = ?', [id]);
...
router.get('/stats/overview', async (req, res) => {
    try {
      ...
      FROM reservations
    `);
        const [reservationsParJour] = await pool.execute(`
      SELECT 
