...
router.get('/', requireAuth, async (req, res) => {
    try {
        ...
        const [preferences] = await pool.execute(`SELECT * FROM user_notification_preferences WHERE user_id = ?`, [user_id]);
        if (preferences.length === 0) {
            // Créer les préférences par défaut si elles n'existent pas
            const [result] = await pool.execute(`INSERT INTO user_notification_preferences (user_id) VALUES (?)`, [user_id]);
            // Récupérer les préférences nouvellement créées
            const [newPreferences] = await pool.execute(`SELECT * FROM user_notification_preferences WHERE user_id = ?`, [user_id]);
...
router.put('/', requireAuth, async (req, res) => {
    try {
        const user_id = req.user.id;
        const { push_enabled, email_enabled, sms_enabled, new_reservation, reservation_confirmed, reservation_cancelled, new_publication, new_like, new_comment, new_follower, promotions, tips } = req.body;
        // Vérifier si les préférences existent
        const [existing] = await pool.execute(`SELECT id FROM user_notification_preferences WHERE user_id = ?`, [user_id]);
        if (existing.length === 0) {
            // Créer les préférences
            await pool.execute(`INSERT INTO user_notification_preferences (
          ...
          promotions, tips
            ...
            await pool.execute(`UPDATE user_notification_preferences SET
        ...
        const [updatedPreferences] = await pool.execute(`SELECT * FROM user_notification_preferences WHERE user_id = ?`, [user_id]);
        res.json({
            ok: true,
...
router.post('/reset', requireAuth, async (req, res) => {
    try {
       ...
       WHERE user_id = ?`, [user_id]);
        ...
        const [resetPreferences] = await pool.execute(`SELECT * FROM user_notification_preferences WHERE user_id = ?`, [user_id]);
        res.json({
            ok: true,
