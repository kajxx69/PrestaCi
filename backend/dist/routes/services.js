...
router.post('/', requireAuth, validateCreateService, async (req, res) => {
    try {
        ...
        const [prestataireRows] = await pool.query('SELECT id FROM prestataires WHERE user_id = ? LIMIT 1', [req.userId]);
        ...
        const [pRows] = await pool.query(`SELECT p.plan_actuel_id, p.abonnement_expires_at, pa.max_services
       FROM prestataires p JOIN plans_abonnement pa ON p.plan_actuel_id = pa.id WHERE p.id = ? LIMIT 1`, [prestataireId]);
        const plan = pRows[0];
        if (!plan)
        ...
        const [countRows] = await pool.query('SELECT COUNT(*) as cnt FROM services WHERE prestataire_id = ?', [prestataireId]);
        ...
        const [result] = await pool.query(`INSERT INTO services (prestataire_id, sous_categorie_id, nom, description, prix, devise, duree_minutes, photos, is_domicile, is_active, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, COALESCE(?, 'FCFA'), ?, ?, COALESCE(?, 0), 1, NOW(), NOW())`, [prestataireId, sous_categorie_id, nom, description || null, prix, devise || null, duree_minutes, photosJson, is_domicile]);
...
router.put('/:id', requireAuth, async (req, res) => {
    try {
        ...
        const [serviceRows] = await pool.query('SELECT id, prestataire_id, nom FROM services WHERE id = ? LIMIT 1', [id]);
        ...
        const photosJson = photos ? JSON.stringify(photos) : null;
        await pool.query(`UPDATE services SET 
         ...
         updated_at = NOW()
       WHERE id = ? AND prestataire_id = ?`, [sous_categorie_id, nom, description, prix, devise, duree_minutes, photosJson, is_domicile, is_active, id, prestataireId]);
        res.json({ ok: true });
    }
    catch (e) {
        res.status(500).json({ error: e.message });
...
router.delete('/:id', requireAuth, async (req, res) => {
    try {
        ...
        const [checkRows] = await pool.query('SELECT id, nom, prestataire_id FROM services WHERE id = ? AND prestataire_id = ? LIMIT 1', [id, prestataireId]);
        ...
        if (checkRows.length === 0) {
            ...
            const [serviceExists] = await pool.query('SELECT id, prestataire_id FROM services WHERE id = ? LIMIT 1', [id]);
        ...
        const [reservationRows] = await pool.query('SELECT COUNT(*) as count FROM reservations WHERE service_id = ?', [id]);
        if (reservationRows[0].count > 0) {
            // Si des réservations existent, désactiver le service au lieu de le supprimer
            await pool.query('UPDATE services SET is_active = 0, updated_at = NOW() WHERE id = ? AND prestataire_id = ?', [id, prestataireId]);
            return res.json({
                ok: true,
        ...
        const [result] = await pool.query('DELETE FROM services WHERE id = ? AND prestataire_id = ?', [id, prestataireId]);
...
router.get('/debug/my-services', requireAuth, async (req, res) => {
    try {
        const userId = req.userId;
        // 1. Récupérer les infos de l'utilisateur
        const [userRows] = await pool.query('SELECT id, email, nom, prenom FROM users WHERE id = ?', [userId]);
        // 2. Récupérer le prestataire associé
        const [prestataireRows] = await pool.query('SELECT id, nom_commercial, user_id FROM prestataires WHERE user_id = ?', [userId]);
        const prestataireId = prestataireRows[0]?.id || null;
        ...
        const [services] = prestataireId
            ? await pool.query(servicesQuery, [prestataireId])
            : [[]];
        // 4. Récupérer TOUS les services pour comparaison (debug uniquement)
        const [allServices] = await pool.query('SELECT id, nom, prestataire_id FROM services ORDER BY id');
        res.json({
            ...
            allServicesInDB: allServices.map((s) => ({
                id: s.id,
                nom: s.nom,
                prestataire_id: s.prestataire_id
            })),
            analysis: {
                correctIsolation: services.every((s) => s.prestataire_id === prestataireId),
                problemServices: services.filter((s) => s.prestataire_id !== prestataireId).map((s) => s.id)
            }
        });
    }
    catch (e) {
...
router.get('/debug/:id', requireAuth, async (req, res) => {
    try {
        ...
        const [prestataireRows] = await pool.query('SELECT id FROM prestataires WHERE user_id = ?', [userId]);
