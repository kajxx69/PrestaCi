...
const parseTemplateVariables = (raw) => {
    if (!raw) {
        return [];
    }
    ...
    try {
        ...
        try {
            ...
            for (let i = 0; i < users.length; i += batchSize) {
                ...
                const batchPromises = batch.map(user => activeConnection.execute(`INSERT INTO notifications (user_id, type, titre, message, data, sent_at) 
             VALUES (?, ?, ?, ?, ?, NOW())`, [user.id, type, cleanTitle, cleanMessage, JSON.stringify(data || {})]));
...
router.post('/targeted', async (req, res) => {
    let connection = null;
    try {
        const { user_ids, title, message, type = 'info', data = {} } = req.body;
        ...
        const [users] = await pool.execute(`SELECT id, nom, email FROM users WHERE id IN (${placeholders})`, validUserIds);
        ...
        await activeConnection.beginTransaction();
        try {
            const insertPromises = validUserIds.map(userId => activeConnection.execute(`INSERT INTO notifications (user_id, type, titre, message, data, sent_at) 
           VALUES (?, ?, ?, ?, ?, NOW())`, [userId, type, cleanTitle, cleanMessage, JSON.stringify(data || {})]));
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        const [notifications] = await pool.execute('SELECT id, titre, user_id FROM notifications WHERE id = ?', [notificationId]);
        ...
        const [result] = await pool.execute('DELETE FROM notifications WHERE id = ?', [notificationId]);
...
router.post('/bulk-delete', async (req, res) => {
    try {
        ...
        const [existingNotifications] = await pool.execute(`SELECT id FROM notifications WHERE id IN (${placeholders})`, validIds);
        ...
        const [result] = await pool.execute(`DELETE FROM notifications WHERE id IN (${placeholders})`, validIds);
        // Log de l'action admin
        try {
            await pool.execute(`INSERT INTO admin_logs (admin_id, action, target_type, target_id, details, created_at) 
...
router.get('/stats/overview', async (req, res) => {
    ...
    `);
        const [notificationsParJour] = await pool.execute(`
      SELECT 
      ...
      ORDER BY date DESC
    `);
        const [utilisateursActifs] = await pool.execute(`
      SELECT 
        u.id as user_id,
        u.nom as user_nom,
        u.email as user_email,
      ...
      LIMIT 20
    `);
        const [typesPopulaires] = await pool.execute(`
      SELECT 
        type,
...
router.get('/templates', async (req, res) => {
    try {
        const [rows] = await pool.execute(`SELECT id, nom, titre, message, variables, is_active, created_at 
...
router.post('/template', async (req, res) => {
    let connection = null;
    try {
        const { template_id, template_nom, variables = {}, target_roles = ['all'], user_ids = null, type } = req.body;
        ...
        const [templateRows] = await pool.execute(`SELECT id, nom, titre, message, variables 
       FROM notification_templates 
       WHERE ${identifierField} = ? AND is_active = 1 
       LIMIT 1`, [identifierValue]);
        ...
        const templateType = (type && ['info', 'success', 'warning', 'error'].includes(type))
            ? type
        ...
        if (user_ids && Array.isArray(user_ids) && user_ids.length > 0) {
            ...
            const placeholders = validUserIds.map(() => '?').join(',');
            const [users] = await pool.execute(`SELECT id FROM users WHERE id IN (${placeholders})`, validUserIds);
            if (users.length !== validUserIds.length) {
                return res.status(400).json({
                    success: false,
                    error: 'Certains utilisateurs n\'existent pas',
        ...
        try {
            ...
            connection = activeConnection;
            await activeConnection.beginTransaction();
            const insertPromises = targetUsers.map(user => activeConnection.execute(`INSERT INTO notifications (user_id, template_id, type, titre, message, data, sent_at) 
           VALUES (?, ?, ?, ?, ?, ?, NOW())`, [user.id, template.id, templateType, processedTitle, processedMessage, JSON.stringify(notificationDataPayload)]));
            await Promise.all(insertPromises);
            await activeConnection.commit();
            try {
                await pool.execute(`INSERT INTO admin_logs (admin_id, action, target_type, target_id, details, created_at) 
...
router.get('/test', async (req, res) => {
    try {
        ...
        const [notifications] = await pool.execute('SELECT COUNT(*) as notification_count FROM notifications');
        res.json({
            success: true,
