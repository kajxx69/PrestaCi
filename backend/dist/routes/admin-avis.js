...
router.get('/', async (req, res) => {
    ...
    `;
        ...
        const [avis] = await pool.execute(query, params);
...
router.get('/:id', async (req, res) => {
    try {
      ...
      WHERE a.id = ?
    `;
        const [avis] = await pool.execute(query, [id]);
        if (avis.length === 0) {
...
router.put('/:id/moderate', async (req, res) => {
    try {
        ...
        const [avis] = await pool.execute(`SELECT a.*, r.prestataire_id, uc.nom as client_nom, s.nom as service_nom
       FROM avis a
       LEFT JOIN reservations r ON a.reservation_id = r.id
       LEFT JOIN users uc ON a.client_id = uc.id
       LEFT JOIN services s ON r.service_id = s.id
...
router.get('/stats/overview', async (req, res) => {
    try {
      ...
      FROM avis
    `);
        const [notesDistribution] = await pool.execute(`
      SELECT 
        note,
      ...
      ORDER BY note DESC
    `);
        const [avisParJour] = await pool.execute(`
      SELECT 
      ...
      ORDER BY date DESC
    `);
        const [servicesLesMieuxNotes] = await pool.execute(`
      SELECT 
        s.nom as service_nom,
      ...
      LIMIT 10
    `);
        const [avisProblematiques] = await pool.execute(`
      SELECT 
        a.id,
        a.note,
        a.commentaire,
...
router.post('/bulk-moderate', async (req, res) => {
    try {
        ...
        const [avis] = await pool.execute(`SELECT a.id, a.client_id, r.prestataire_id, uc.nom as client_nom, s.nom as service_nom
       FROM avis a
       LEFT JOIN reservations r ON a.reservation_id = r.id
       LEFT JOIN users uc ON a.client_id = uc.id
       LEFT JOIN services s ON r.service_id = s.id
       WHERE a.id IN (${placeholders})`, avis_ids);
       ...
       SET is_moderated = 1, is_approved = ?, moderation_reason = ?, 
           moderated_by = ?, moderated_at = NOW(), updated_at = NOW()
       WHERE id IN (${placeholders})`, [is_approved, reason || null, req.user.id, ...avis_ids]);
        // Log de l'action admin
        await pool.execute(`INSERT INTO admin_logs (admin_id, action, target_type, target_id, details, created_at) 
       VALUES (?, ?, 'avis', 0, ?, NOW())`, [
            req.user.id,
