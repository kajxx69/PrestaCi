...
router.get('/status', async (req, res) => {
    ...
    `);
        ...
        const [diskUsage] = await pool.execute(`
      SELECT 
    ...
    `);
        ...
        const [backgroundTasks] = await pool.execute(`
      SELECT 
    ...
    try {
        ...
        if (cache_type === 'all' || cache_type === 'sessions') {
            try {
                ...
                try {
                    const [columns] = await pool.query(`SHOW COLUMNS FROM user_sessions LIKE 'last_activity'`);
                    if (Array.isArray(columns) && columns.length > 0) {
                        whereClause += ' OR last_activity < DATE_SUB(NOW(), INTERVAL 30 DAY)';
                    }
                }
                catch (columnError) {
                    console.warn('Impossible de vérifier la colonne last_activity:', columnError === null || columnError === void 0 ? void 0 : columnError.message);
                }
                await pool.execute(`DELETE FROM user_sessions ${whereClause}`);
                clearedCaches.push('expired_sessions');
            }
            catch (sessionError) {
                console.warn('Nettoyage des sessions ignoré:', sessionError === null || sessionError === void 0 ? void 0 : sessionError.message);
            }
        }
...
router.get('/logs', async (req, res) => {
    try {
        const { level = 'all', search = '', page = 1, limit = 50, date_debut, date_fin } = req.query;
...
router.post('/mode', async (req, res) => {
    try {
        ...
        await pool.execute(`INSERT INTO app_settings (cle, valeur, type, description, updated_at) 
       VALUES ('maintenance_mode', ?, 'boolean', 'Mode maintenance activé/désactivé', NOW())
       ON DUPLICATE KEY UPDATE 
       valeur = VALUES(valeur), updated_at = VALUES(updated_at)`, [enabled ? '1' : '0']);
        if (enabled) {
            // Sauvegarder le message et la durée estimée
            await pool.execute(`INSERT INTO app_settings (cle, valeur, type, description, updated_at) 
         VALUES ('maintenance_message', ?, 'string', 'Message affiché pendant la maintenance', NOW())
         ON DUPLICATE KEY UPDATE 
         valeur = VALUES(valeur), updated_at = VALUES(updated_at)`, [message || 'Maintenance en cours...']);
            if (estimated_duration) {
                await pool.execute(`INSERT INTO app_settings (cle, valeur, type, description, updated_at) 
           VALUES ('maintenance_duration', ?, 'integer', 'Durée estimée de la maintenance', NOW())
           ON DUPLICATE KEY UPDATE 
           valeur = VALUES(valeur), updated_at = VALUES(updated_at)`, [estimated_duration.toString()]);
            }
        }
        // Log de l'action admin
        await pool.execute(`INSERT INTO admin_logs (admin_id, action, target_type, target_id, details, created_at) 
