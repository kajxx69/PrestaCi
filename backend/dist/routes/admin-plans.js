...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const [plans] = await pool.execute(`SELECT 
        p.*,
        COUNT(DISTINCT t.id) as nombre_abonnements_actifs,
      ...
      GROUP BY p.id`, [id]);
        ...
        const [recentSubscriptions] = await pool.execute(`SELECT 
        s.*,
        u.nom as user_nom,
...
router.post('/', async (req, res) => {
    try {
        const { nom, description, prix, duree_mois = 1, max_services, max_photos_par_service = 5, commission_percentage = 0, features = [], is_popular = false } = req.body;
        // Validation
        ...
        const [existing] = await pool.execute('SELECT id FROM plans_abonnement WHERE nom = ?', [nom]);
        ...
        if (is_popular) {
            await pool.execute('UPDATE plans_abonnement SET is_popular = 0 WHERE is_popular = 1');
        }
        // Créer le plan
        const [result] = await pool.execute(`INSERT INTO plans_abonnement 
...
router.put('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { nom, description, prix, duree_mois, max_services, max_photos_par_service, commission_percentage, features, is_active, is_popular } = req.body;
        // Vérifier que le plan existe
        const [existing] = await pool.execute('SELECT id, nom FROM plans_abonnement WHERE id = ?', [id]);
        if (existing.length === 0) {
            return res.status(404).json({ error: 'Plan non trouvé' });
        }
        // Vérifier que le nouveau nom n'existe pas déjà (si changé)
        if (nom && nom !== existing[0].nom) {
            const [duplicate] = await pool.execute('SELECT id FROM plans_abonnement WHERE nom = ? AND id != ?', [nom, id]);
        ...
        // Si ce plan est marqué comme populaire, retirer le flag des autres
        if (is_popular) {
            await pool.execute('UPDATE plans_abonnement SET is_popular = 0 WHERE is_popular = 1 AND id != ?', [id]);
        }
        ...
        params.push(id);
        await pool.execute(`UPDATE plans_abonnement SET ${updates.join(', ')} WHERE id = ?`, params);
        // Log de l'action admin
        await pool.execute(`INSERT INTO admin_logs (admin_id, action, target_type, target_id, details, created_at) 
...
router.delete('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        // Vérifier que le plan existe
        const [plans] = await pool.execute('SELECT id, nom FROM plans_abonnement WHERE id = ?', [id]);
        if (plans.length === 0) {
            return res.status(404).json({ error: 'Plan non trouvé' });
        }
        // Vérifier s'il y a des abonnements actifs
        const [activeSubscriptions] = await pool.execute('SELECT COUNT(*) as count FROM subscriptions WHERE plan_id = ? AND statut = "actif"', [id]);
...
router.put('/subscriptions/:id/status', async (req, res) => {
    try {
        ...
        const [subscriptions] = await pool.execute(`SELECT s.*, u.nom as user_nom, p.nom as plan_nom
       FROM transactions_wave s
       LEFT JOIN users u ON s.prestataire_id = u.id
       LEFT JOIN plans_abonnement p ON s.plan_id = p.id
       WHERE s.id = ?`, [id]);
        ...
        await pool.execute('UPDATE subscriptions SET statut = ?, updated_at = NOW() WHERE id = ?', [statut, id]);
        // Log de l'action admin
        await pool.execute(`INSERT INTO admin_logs (admin_id, action, target_type, target_id, details, created_at) 
...
router.get('/stats/overview', async (req, res) => {
    ...
    `);
      ...
      LEFT JOIN plans_abonnement p ON s.plan_id = p.id
    `);
        const [planPopularity] = await pool.execute(`
      SELECT 
        p.nom as plan_nom,
        p.prix as plan_prix,
    ...
    `);
        const [churnAnalysis] = await pool.execute(`
      SELECT 
