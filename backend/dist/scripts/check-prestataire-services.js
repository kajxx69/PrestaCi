...
async function checkPrestataireServices() {
    try {
        ...
        const [prestataires] = await pool.query(`
      SELECT 
    ...
    `);
        ...
        for (const prestataire of prestataires) {
            if (prestataire.nombre_services > 0) {
                const [services] = await pool.query('SELECT id, nom, is_active, created_at FROM services WHERE prestataire_id = ?', [prestataire.prestataire_id]);
