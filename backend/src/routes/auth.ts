...
async function getSessionDurationHours(): Promise<number> {
  try {
    const [rows]: any = await pool.query('SELECT valeur FROM configurations WHERE cle = ? LIMIT 1', ['session_duration_hours']);
...
router.post('/register', validateRegister, async (req: Request, res: Response) => {
  try {
    ...
    const [result]: any = await pool.query(
      'INSERT INTO users (email, password_hash, role_id, nom, prenom, telephone) VALUES (?, ?, ?, ?, ?, ?)',
      [email, password_hash, role_id, nom, prenom, telephone || null]
    );
    ...
    if (role_id === 2) {
      await pool.query(
        'INSERT INTO prestataires (user_id, nom_commercial, ville, adresse, pays, latitude, longitude) VALUES (?, ?, ?, ?, ?, ?, ?)',
        [userId, nom_commercial, ville, adresse, 'Côte d\'Ivoire', latitude || null, longitude || null]
      );
    }

    const [userRows]: any = await pool.query(
      'SELECT u.id, u.email, u.role_id, u.nom, u.prenom, u.telephone, r.nom AS role_nom FROM users u JOIN roles r ON u.role_id = r.id WHERE u.id = ? LIMIT 1',
      [userId]
    );
    ...
    await pool.query(
      'INSERT INTO user_sessions (user_id, token, device_info, ip_address, expires_at) VALUES (?, ?, ?, ?, DATE_ADD(NOW(), INTERVAL ? HOUR))',
      [user.id, token, req.headers['user-agent'] || '', req.ip || '', hours]
    );
...
router.post('/login', validateLogin, async (req: Request, res: Response) => {
  try {
    ...
    const [roleRows]: any = await pool.query('SELECT nom FROM roles WHERE id = ? LIMIT 1', [user.role_id]);
