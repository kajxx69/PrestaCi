...
router.get('/', requireAuth, requireRole('admin'), async (req, res) => {
  try {
    ...
    const [countResult] = await pool.execute<RowDataPacket[]>(
      `SELECT COUNT(*) as total FROM transactions_wave tw ${whereClause}`,
      params
    );
...
router.put('/:id/validate', requireAuth, requireRole('admin'), async (req, res) => {
  try {
    ...
    const [transactions] = await pool.execute<RowDataPacket[]>(
      'SELECT * FROM transactions_wave WHERE id = ? AND statut = "en_attente"',
      [transactionId]
    );
    ...
    try {
      ...
      await pool.execute(
        `UPDATE prestataires 
         ...
         WHERE id = ?`,
        [transaction.plan_id, transaction.duree_abonnement_jours, transaction.prestataire_id]
      );
  ...
  try {
    ...
    await pool.execute(
      `UPDATE transactions_wave 
       ...
       WHERE id = ?`,
      [adminId, motif_rejet.trim(), transactionId]
    );
