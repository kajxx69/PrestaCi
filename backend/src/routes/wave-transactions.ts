...
router.post('/', requireAuth, async (req, res) => {
  try {
    ...
    const [prestataireRows] = await pool.execute<RowDataPacket[]>(
    ...
    const [existingTransactions] = await pool.execute<RowDataPacket[]>(
      'SELECT * FROM transactions_wave WHERE prestataire_id = ? AND statut = "en_attente"',
      [prestataire_id]
    );
    ...
    const [result] = await pool.execute<ResultSetHeader>(
      `INSERT INTO transactions_wave (
        prestataire_id, plan_id, transaction_id_wave, montant, devise, 
        duree_abonnement_jours, statut
      ) VALUES (?, ?, ?, ?, ?, ?, 'en_attente')`,
      [prestataire_id, plan_id, transaction_id_wave, montant, devise, duree_abonnement_jours]
    );
