...
router.get('/', async (req, res) => {
  try {
    ...
    const [categories] = await pool.execute<CategoryRow[]>(query);
...
router.get('/:id', async (req, res) => {
  try {
    ...
    const [sousCategories] = await pool.execute<SubCategoryRow[]>(
      `SELECT 
        sc.*,
      ...
      GROUP BY sc.id
      ORDER BY sc.ordre_affichage ASC, sc.nom ASC`,
      [id]
    );
...
router.post('/', async (req, res) => {
  try {
    ...
    const [result] = await pool.execute<ResultSetHeader>(
      `INSERT INTO categories (nom, description, icone, couleur, is_active, created_at, updated_at)
       VALUES (?, ?, ?, ?, 1, NOW(), NOW())`,
      [nom, description, icone || 'folder', couleur || '#3B82F6']
    );
  ...
  try {
    ...
    const [services] = await pool.execute<RowDataPacket[]>(
      `SELECT COUNT(*) as count FROM services s
       ...
       WHERE sc.categorie_id = ? AND s.deleted_at IS NULL`,
      [id]
    );
    ...
    await pool.execute(
      'DELETE FROM sous_categories WHERE categorie_id = ?',
      [id]
    );
...
router.get('/subcategories/all', async (req, res) => {
  try {
    ...
    const query = `
      ...
      ${whereClause}
      GROUP BY sc.id
      ORDER BY c.nom ASC, sc.ordre_affichage ASC, sc.nom ASC
    `;

    const [sousCategories] = await pool.execute<SubCategoryRow[]>(query, params);
...
router.post('/:id/subcategories', async (req, res) => {
  try {
    ...
    const [orderRows] = await pool.execute<RowDataPacket[]>(
      'SELECT COALESCE(MAX(ordre_affichage), 0) + 1 AS next_order FROM sous_categories WHERE categorie_id = ?',
      [categorieId]
    );
    ...
    const [result] = await pool.execute<ResultSetHeader>(
      `INSERT INTO sous_categories (nom, description, categorie_id, ordre_affichage, is_active, created_at, updated_at)
       VALUES (?, ?, ?, ?, 1, NOW(), NOW())`,
      [nom, description, categorieId, nextOrder]
    );
...
router.put('/subcategories/:id', async (req, res) => {
  try {
    ...
    const [existing] = await pool.execute<RowDataPacket[]>(
      'SELECT id, nom, categorie_id FROM sous_categories WHERE id = ?',
      [id]
    );
    ...
    if (nom) {
      ...
      if (nom !== existing[0].nom) {
        const [duplicate] = await pool.execute<RowDataPacket[]>(
          'SELECT id FROM sous_categories WHERE nom = ? AND categorie_id = ? AND id != ?',
          [nom, existing[0].categorie_id, id]
        );
    ...
    if (ordre_affichage !== undefined) {
      updates.push('ordre_affichage = ?');
      params.push(ordre_affichage);
    }
    ...
    await pool.execute(
      `UPDATE sous_categories SET ${updates.join(', ')} WHERE id = ?`,
      params
    );
    ...
    await pool.execute(
      ...
      [
        req.user.id,
        id,
        JSON.stringify({ nom, description, is_active, icone, ordre_affichage })
      ]
    );
...
// PUT /api/admin/categories/:id/subcategories/reorder - Réordonner les sous-catégories
router.put('/:id/subcategories/reorder', async (req, res) => {
  ...
  try {
    ...
    if (!Array.isArray(order) || order.length === 0) {
      connection.release();
      return res.status(400).json({ error: 'Ordre invalide fourni' });
    }

    const ids = order.map(item => item.id).filter(id => typeof id === 'number');
    if (ids.length !== order.length) {
      connection.release();
      return res.status(400).json({ error: 'Identifiants de sous-catégorie manquants' });
    }

    const placeholders = ids.map(() => '?').join(',');
    const [existing] = await connection.execute<RowDataPacket[]>(
      `SELECT id FROM sous_categories WHERE categorie_id = ? AND id IN (${placeholders})`,
      [categorieId, ...ids]
    );

    if (existing.length !== ids.length) {
      connection.release();
      return res.status(400).json({ error: 'Certaines sous-catégories sont invalides' });
    }
    ...
    for (let index = 0; index < order.length; index++) {
      ...
      await connection.execute(
        'UPDATE sous_categories SET ordre_affichage = ?, updated_at = NOW() WHERE id = ? AND categorie_id = ?',
        [finalOrder, item.id, categorieId]
      );
    }

    await connection.execute(
      `INSERT INTO admin_logs (admin_id, action, target_type, target_id, details, created_at) 
       VALUES (?, 'subcategory_reordered', 'category', ?, ?, NOW())`,
      [
        req.user.id,
        categorieId,
        JSON.stringify({ categorie_id: categorieId, order })
      ]
    );
...
router.delete('/subcategories/:id', async (req, res) => {
  try {
    ...
    // Vérifier que la sous-catégorie existe
    const [sousCategories] = await pool.execute<RowDataPacket[]>(
      'SELECT id, nom FROM sous_categories WHERE id = ?',
      [id]
    );
    ...
    // Vérifier s'il y a des services associés
    const [services] = await pool.execute<RowDataPacket[]>(
      'SELECT COUNT(*) as count FROM services WHERE sous_categorie_id = ? AND deleted_at IS NULL',
      [id]
    );
