...
router.post('/', requireAuth, async (req, res) => {
  try {
    ...
    const [reservations] = await pool.execute<RowDataPacket[]>(
      `SELECT r.id, r.prestataire_id, r.service_id, r.statut_id, s.nom as statut_nom
       FROM reservations r
    ...
    const [existingAvis] = await pool.execute<RowDataPacket[]>(
    ...
    const [result] = await pool.execute<ResultSetHeader>(
      `INSERT INTO avis (reservation_id, client_id, prestataire_id, note, commentaire, photos)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [
        reservation_id,
        client_id,
...
router.get('/prestataire/:id', async (req, res) => {
  try {
    ...
    const avisWithPhotos = avis.map((avisItem: any) => ({
      ...avisItem,
...
router.delete('/:id', requireAuth, async (req, res) => {
  try {
    ...
    const [avis] = await pool.execute<RowDataPacket[]>(
      'SELECT prestataire_id, reservation_id FROM avis WHERE id = ? AND client_id = ?',
      [avis_id, client_id]
    );
    ...
    const [[reservation]] = await pool.execute<RowDataPacket[]>(
      'SELECT service_id FROM reservations WHERE id = ?',
      [avis[0].reservation_id]
    );

    const serviceId = reservation?.service_id || null;
    ...
    // Mettre à jour les moyennes
    if (serviceId) {
      await updateAverageRatings(avis[0].prestataire_id, serviceId);
    }
  ...
  try {
    ...
    await pool.execute(
      `UPDATE prestataires 
       ...
       WHERE id = ?`,
      [prestataire_id, prestataire_id, prestataire_id]
    );
