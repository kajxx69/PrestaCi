...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    const params: any[] = mine ? [userId] : [];

    const [rows]: any = await pool.query(
      `SELECT pub.id, pub.client_id, pub.prestataire_id, pub.service_id, pub.description, pub.photos, pub.videos,
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const [result]: any = await pool.query(
      `INSERT INTO publications (client_id, prestataire_id, service_id, description, photos, videos, nombre_likes, is_visible, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, 0, 1, NOW(), NOW())`,
      [userId, prestataire_id, service_id || null, description, photosJson, videosJson]
    );
...
router.post('/:id/like', async (req: Request, res: Response) => {
  try {
    ...
    const [result]: any = await pool.query(`INSERT IGNORE INTO likes (publication_id, user_id, created_at) VALUES (?, ?, NOW())`, [pubId, userId]);
...
router.delete('/:id/like', async (req: Request, res: Response) => {
  try {
    ...
    const [del]: any = await pool.query(`DELETE FROM likes WHERE publication_id = ? AND user_id = ?`, [pubId, userId]);
...
router.get('/:id/comments', async (req: Request, res: Response) => {
  try {
    const pubId = Number(req.params.id);
    const [rows]: any = await pool.query(
      `SELECT c.id, c.contenu, c.created_at, u.prenom AS client_prenom, u.nom AS client_nom, u.photo_profil
         FROM commentaires c
...
router.post('/:id/comments', async (req: Request, res: Response) => {
  try {
    ...
    const [result]: any = await pool.query(
      'INSERT INTO commentaires (publication_id, user_id, contenu, created_at) VALUES (?, ?, ?, NOW())',
      [pubId, userId, contenu]
    );
