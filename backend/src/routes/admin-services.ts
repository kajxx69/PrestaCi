...
router.get('/', async (req, res) => {
  try {
    ...
    const [services] = await pool.execute<ServiceRow[]>(query, params);
  ...
  try {
    ...
    const query = `
      SELECT 
        ...
        sc.nom as sous_categorie_nom,
        COUNT(DISTINCT r.id) as total_reservations,
        COUNT(DISTINCT CASE WHEN sr.nom = 'terminee' THEN r.id END) as reservations_terminees,
        COALESCE(AVG(a.note), 0) as moyenne_avis,
      ...
      LEFT JOIN categories c ON sc.categorie_id = c.id
      LEFT JOIN reservations r ON s.id = r.service_id
      LEFT JOIN statuts_reservation sr ON r.statut_id = sr.id
      LEFT JOIN avis a ON r.id = a.reservation_id
      WHERE s.id = ?
      GROUP BY s.id
    `;

    const [services] = await pool.execute<ServiceRow[]>(query, [id]);
...
router.put('/:id/status', async (req, res) => {
  try {
    ...
    const [services] = await pool.execute<RowDataPacket[]>(
      'SELECT id, prestataire_id FROM services WHERE id = ?',
      [id]
    );
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    const [services] = await pool.execute<RowDataPacket[]>(
      'SELECT id, prestataire_id, nom FROM services WHERE id = ?',
      [id]
    );
    ...
    const [activeReservations] = await pool.execute<RowDataPacket[]>(
      `SELECT COUNT(*) as count FROM reservations 
       WHERE service_id = ? AND statut IN ('en_attente', 'confirmee')`,
      [id]
    );
...
router.get('/stats/overview', async (req, res) => {
  try {
    ...
    const [topCategories] = await pool.execute<RowDataPacket[]>(`
      SELECT 
        c.nom as categorie,
    ...
    const [topPrestataires] = await pool.execute<RowDataPacket[]>(`
      SELECT 
