...
async function getPrestataireIdByUserId(userId: number): Promise<number | null> {
  const [rows]: any = await pool.query('SELECT id FROM prestataires WHERE user_id = ? LIMIT 1', [userId]);
...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    const params: any[] = [prestataireId];
...
router.put('/:id/accept', async (req: Request, res: Response) => {
  try {
    ...
    const [reservationRows]: any = await pool.query(
      `SELECT r.*, sr.nom as statut_nom, s.nom as service_nom 
       FROM reservations r 
       JOIN statuts_reservation sr ON r.statut_id = sr.id 
       JOIN services s ON r.service_id = s.id
       WHERE r.id = ? AND r.prestataire_id = ? LIMIT 1`,
    ...
    const [statutRows]: any = await pool.query("SELECT id FROM statuts_reservation WHERE nom = 'confirmee' LIMIT 1");
    ...
    await pool.query(
      'UPDATE reservations SET statut_id = ?, updated_at = NOW() WHERE id = ?',
      [confirmedStatusId, reservationId]
    );
    ...
    await pool.query(
      `INSERT INTO historique_reservations (reservation_id, ancien_statut_id, nouveau_statut_id, commentaire, changed_by_user_id)
       VALUES (?, ?, ?, ?, ?)`,
      [reservationId, reservation.statut_id, confirmedStatusId, 'Réservation acceptée par le prestataire', userId]
    );
    
    // Envoyer notification au client
    try {
      const [prestataireRows]: any = await pool.query(
        'SELECT u.nom, u.prenom FROM users u JOIN prestataires p ON u.id = p.user_id WHERE p.id = ? LIMIT 1',
        [prestataireId]
      );
      ...
      if (prestataireRows.length > 0) {
        ...
        await ClientNotifications.reservationConfirmee(
        ...
        await ClientInAppNotifications.reservationConfirmee(
          reservation.client_id,
          prestataireNom,
          serviceNom,
          dateReservation,
          reservation.heure_debut || 'Non spécifiée'
        );
      }
    } catch (notifError) {
...
router.put('/:id/reject', async (req: Request, res: Response) => {
  try {
    ...
    const [statutRows]: any = await pool.query("SELECT id FROM statuts_reservation WHERE nom = 'refusee' LIMIT 1");
    if (statutRows.length === 0) {
    ...
    await pool.query(
      'UPDATE reservations SET statut_id = ?, updated_at = NOW() WHERE id = ?',
      [rejectedStatusId, reservationId]
    );
    ...
    await pool.query(
      `INSERT INTO historique_reservations (reservation_id, ancien_statut_id, nouveau_statut_id, commentaire, changed_by_user_id)
       VALUES (?, ?, ?, ?, ?)`,
      [reservationId, reservation.statut_id, rejectedStatusId, commentaire, userId]
    );
    ...
    try {
      ...
      if (prestataireRows.length > 0) {
        const prestataireNom = `${prestataireRows[0].prenom} ${prestataireRows[0].nom}`;
        const serviceNom = reservation.service_nom || 'votre service';
        await ClientNotifications.reservationRefusee(
          reservation.client_id,
          prestataireNom,
          serviceNom,
          motif
        );
        await ClientInAppNotifications.reservationRefusee(
          reservation.client_id,
          prestataireNom,
          serviceNom,
          motif
...
router.put('/:id/complete', async (req: Request, res: Response) => {
  try {
    ...
    const [statutRows]: any = await pool.query("SELECT id FROM statuts_reservation WHERE nom = 'terminee' LIMIT 1");
    if (statutRows.length === 0) {
    ...
    await pool.query(
      'UPDATE reservations SET statut_id = ?, updated_at = NOW() WHERE id = ?',
      [completedStatusId, reservationId]
    );
    ...
    await pool.query(
      `INSERT INTO historique_reservations (reservation_id, ancien_statut_id, nouveau_statut_id, commentaire, changed_by_user_id)
       VALUES (?, ?, ?, ?, ?)`,
      [reservationId, reservation.statut_id, completedStatusId, 'Service terminé', userId]
    );
    ...
    try {
      ...
      if (prestataireRows.length > 0) {
        const prestataireNom = `${prestataireRows[0].prenom} ${prestataireRows[0].nom}`;
        const serviceNom = reservation.service_nom || 'votre service';
        await ClientNotifications.serviceTermine(
          reservation.client_id,
          prestataireNom,
          serviceNom
        );
        await ClientInAppNotifications.serviceTermine(
          reservation.client_id,
          prestataireNom,
          serviceNom
        );
