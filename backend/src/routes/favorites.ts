...
router.get('/providers', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    const [rows]: any = await pool.query(
      `SELECT fp.prestataire_id AS id, p.nom_commercial, p.ville, p.bio, p.adresse, p.photos_etablissement, p.is_verified,
...
router.get('/services', requireAuth, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).user.id;
    const [rows]: any = await pool.query(
      `SELECT fs.service_id AS id, sv.nom, sv.description, sv.photos, sv.prix, sv.devise, sv.duree_minutes,
...
router.get('/publications', requireAuth, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).user.id;
    const [rows]: any = await pool.query(
      `SELECT fp.publication_id AS id, pub.description, pub.photos, pub.videos, pub.nombre_likes, pub.created_at,
