...
router.get('/revenue', async (req, res) => {
  try {
    ...
    `;
    const [revenueByPeriod] = await pool.execute<RowDataPacket[]>(query);
    ...
    `;
    const [revenueByCategory] = await pool.execute<RowDataPacket[]>(categoryQuery);
    ...
    const topProvidersQuery = `
      ...
      LIMIT 20
    `;
    const [topProviders] = await pool.execute<RowDataPacket[]>(topProvidersQuery);
...
router.get('/users', async (req, res) => {
  try {
    ...
    const [registrationsByDay] = await pool.execute<RowDataPacket[]>(`
      SELECT 
...
router.get('/services', async (req, res) => {
  try {
    ...
    const [popularServices] = await pool.execute<RowDataPacket[]>(`
      SELECT 
    ...
    const [categoryDistribution] = await pool.execute<RowDataPacket[]>(`
      SELECT 
        c.nom as categorie,
    ...
    // Analyse des prix
    const [priceAnalysis] = await pool.execute<RowDataPacket[]>(`
      SELECT 
        c.nom as categorie,
    ...
    const [recentServices] = await pool.execute<RowDataPacket[]>(`
      SELECT 
        s.nom as service_nom,
        u.nom as prestataire_nom,
        c.nom as categorie_nom,
...
router.get('/performance', async (req, res) => {
  try {
    ...
    const [conversionMetrics] = await pool.execute<RowDataPacket[]>(`
      SELECT 
    ...
    const [retentionAnalysis] = await pool.execute<RowDataPacket[]>(`
      SELECT 
    ...
    const [timeAnalysis] = await pool.execute<RowDataPacket[]>(`
      SELECT 
    ...
    const [confirmationDelays] = await pool.execute<RowDataPacket[]>(`
      SELECT 
