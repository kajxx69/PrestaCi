...
async function createTestServices() {
  try {
    ...
    const [userRows]: any = await pool.query(
      'SELECT u.id, u.email, p.id as prestataire_id FROM users u JOIN prestataires p ON u.id = p.user_id WHERE u.email = ?',
      [userEmail]
    );
    ...
    if (userRows.length === 0) {
      ...
      const [prestataires]: any = await pool.query(
        'SELECT u.email FROM users u JOIN prestataires p ON u.id = p.user_id'
      );
      prestataires.forEach((p: any) => console.log(`  - ${p.email}`));
      process.exit(1);
    }
    ...
    const [subCategories]: any = await pool.query(
      'SELECT id, nom FROM sous_categories LIMIT 1'
    );
    ...
    if (subCategories.length === 0) {
      ...
      const [catResult]: any = await pool.query(
        'INSERT INTO categories (nom, description, icone, couleur, is_active) VALUES (?, ?, ?, ?, ?)',
        ['Services Test', 'Catégorie de test', 'settings', '#3B82F6', 1]
      );
      
      const [subCatResult]: any = await pool.query(
        'INSERT INTO sous_categories (categorie_id, nom, description, is_active) VALUES (?, ?, ?, ?)',
        [catResult.insertId, 'Sous-catégorie Test', 'Pour les tests', 1]
      );
    ...
    const [allServices]: any = await pool.query(
      'SELECT id, nom, prix, devise, is_active FROM services WHERE prestataire_id = ?',
      [user.prestataire_id]
    );
